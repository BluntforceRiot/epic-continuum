...
def installed_toolchain() -> dict[str, str]:
    ...
    for distribution in ("pip", "setuptools", "wheel", "build", "twine"):
        try:
            ...
            raise RuntimeError(f"canonical build dependency is not installed: {distribution}") from exc
    return versions
...
def _read_source_provenance(source_zip: Path) -> tuple[bytes, dict[str, Any]]:
    with zipfile.ZipFile(source_zip) as archive:
        names = archive.namelist()
        _assert_unique(names, artifact=source_zip)
        candidates = [name for name in names if name.count("/") == 1 and name.endswith(f"/{PROVENANCE_NAME}")]
        if len(candidates) != 1:
            raise RuntimeError(f"{source_zip.name} must contain exactly one root release provenance file")
    ...
    if any(not name.startswith(expected_root) for name in names):
        raise RuntimeError("source ZIP contains a member outside its canonical version root")
    ...
    with zipfile.ZipFile(source_zip) as manifest_archive:
        ...
        for info in manifest_archive.infolist():
            if info.filename == expected_root or info.filename in excluded:
                continue
            mode = (info.external_attr >> 16) & 0xFFFF
            file_type = stat.S_IFMT(mode)
            if file_type not in {stat.S_IFREG, stat.S_IFDIR}:
                raise RuntimeError("source ZIP contains a non-file, non-directory member")
            is_directory = info.is_dir()
            if is_directory != (file_type == stat.S_IFDIR):
                raise RuntimeError("source ZIP member mode and directory marker disagree")
            row: dict[str, object] = {
                "path": info.filename,
                "mode": f"{mode:o}",
            ...
            if not is_directory:
                member_bytes = manifest_archive.read(info.filename)
    ...
    if len(commit) != 40 or any(character not in "0123456789abcdef" for character in commit):
        mismatches["git_commit"] = {"expected": "40 lowercase hexadecimal characters", "actual": commit}
    if isinstance(epoch, bool) or not isinstance(epoch, int) or epoch < 0:
        mismatches["source_date_epoch"] = {"expected": "non-negative integer", "actual": epoch}
    ...
    if mismatches:
        raise RuntimeError(f"source ZIP provenance does not match its members: {mismatches}")
    return root_bytes, payload


def _read_wheel_provenance(wheel_path: Path, *, version: str) -> tuple[bytes, str]:
    expected_filename = f"{EXPECTED_NORMALIZED_NAME}-{version}-py3-none-any.whl"
    if wheel_path.name != expected_filename:
        raise RuntimeError("wheel filename does not match the canonical distribution identity")
    metadata_root = f"{EXPECTED_NORMALIZED_NAME}-{version}.dist-info"
    with zipfile.ZipFile(wheel_path) as archive:
        names = archive.namelist()
        _assert_unique(names, artifact=wheel_path)
        provenance = archive.read(WHEEL_PROVENANCE_NAME)
        wheel_metadata_name = f"{metadata_root}/WHEEL"
        package_metadata_name = f"{metadata_root}/METADATA"
        if wheel_metadata_name not in names or package_metadata_name not in names:
            raise RuntimeError(f"{wheel_path.name} is missing canonical WHEEL or METADATA")
        if sum(name.endswith(".dist-info/WHEEL") for name in names) != 1:
            raise RuntimeError(f"{wheel_path.name} must contain exactly one WHEEL metadata file")
    ...
    generator_lines = [line for line in wheel_metadata.splitlines() if line.startswith("Generator: ")]
    ...
    if str(package_metadata.get("Name") or "") != EXPECTED_DISTRIBUTION_NAME:
        raise RuntimeError("wheel METADATA Name does not match the canonical distribution")
    if str(package_metadata.get("Version") or "") != version:
        raise RuntimeError("wheel METADATA Version does not match source provenance")
    return provenance, generator_lines[0].removeprefix("Generator: ")


def _read_sdist_provenance(sdist_path: Path, *, version: str) -> bytes:
    expected_root = f"{EXPECTED_NORMALIZED_NAME}-{version}"
    if sdist_path.name != f"{expected_root}.tar.gz":
        raise RuntimeError("sdist filename does not match the canonical distribution identity")
    with tarfile.open(sdist_path, "r:gz") as archive:
        names = archive.getnames()
        _assert_unique(names, artifact=sdist_path)
        if any(name != expected_root and not name.startswith(f"{expected_root}/") for name in names):
            raise RuntimeError("sdist contains a member outside its canonical version root")
        ...
        if provenance_name not in names or package_metadata_name not in names:
            raise RuntimeError(f"{sdist_path.name} is missing canonical provenance or PKG-INFO")
        ...
        package_metadata = BytesParser(policy=policy.default).parsebytes(metadata_file.read())
    if str(package_metadata.get("Name") or "") != EXPECTED_DISTRIBUTION_NAME:
        raise RuntimeError("sdist PKG-INFO Name does not match the canonical distribution")
    if str(package_metadata.get("Version") or "") != version:
...
def _assert_distinct_release_paths(
    *,
    ...
    if collisions:
        ...
        raise RuntimeError(f"release input/output paths must be pairwise distinct: {rendered}")


def _require_canonical_output_names(*, receipt_out: Path, checksums_out: Path) -> None:
    mismatches: list[str] = []
    if receipt_out.name != RECEIPT_NAME:
        mismatches.append(f"receipt_out={receipt_out.name!r} (expected {RECEIPT_NAME!r})")
    if checksums_out.name != CHECKSUMS_NAME:
        mismatches.append(f"checksums_out={checksums_out.name!r} (expected {CHECKSUMS_NAME!r})")
    if mismatches:
        raise RuntimeError(
...
def verify_bound_release(
    ...
    *,
    ...
    if not isinstance(receipt, dict) or receipt.get("schema") != (
        "epic_continuum.release_distribution_receipt.v1"
    ):
    ...
    if receipt.get("distribution_build_toolchain") != toolchain:
        raise RuntimeError("release receipt toolchain does not match the active canonical toolchain")
    reproducibility = receipt.get("reproducibility")
    if reproducibility != {"independent_build_count": 2, "byte_identical": True}:
        raise RuntimeError("release receipt does not attest two byte-identical canonical builds")
    ...
    for row in artifact_rows:
        ...
        if digest != row.get("sha256") or len(payload) != row.get("size_bytes"):
            raise RuntimeError(f"release receipt artifact bytes do not match: {filename}")
        by_kind[kind] = path
    ...
    if manifest != expected_checksums:
        raise RuntimeError("SHA256SUMS does not exactly bind the receipt and release artifacts")
    ...
    source_provenance, provenance = _read_source_provenance(by_kind["source_zip"])
    version = str(provenance.get("version") or "")
    wheel_provenance, wheel_generator = _read_wheel_provenance(
        by_kind["wheel"], version=version
    )
    require_canonical_wheel_generator(wheel_generator)
    sdist_provenance = _read_sdist_provenance(by_kind["sdist"], version=version)
...
def finalize_distributions(
    *,
    ...
    if not comparison_dir.is_dir():
        raise RuntimeError(f"independent build directory does not exist: {comparison_dir}")

    toolchain = require_canonical_toolchain(actual_toolchain)
    source_provenance, provenance = _read_source_provenance(source_zip)
    version = str(provenance.get("version") or "")
    wheel_provenance, wheel_generator = _read_wheel_provenance(
        wheel_path, version=version
    )
    sdist_provenance = _read_sdist_provenance(sdist_path, version=version)
    if wheel_provenance != source_provenance or sdist_provenance != source_provenance:
        raise RuntimeError("wheel or sdist provenance does not match the canonical source ZIP")
    if provenance.get("distribution_build_toolchain") != CANONICAL_TOOLCHAIN:
        raise RuntimeError("source provenance does not name the canonical distribution toolchain")
    require_canonical_wheel_generator(wheel_generator)
    ...
    receipt = {
        "schema": "epic_continuum.release_distribution_receipt.v1",
...
def main() -> int:
    ...
    if args.verify_directory is not None:
        if any(value is not None for value in finalization_values.values()):
            parser.error("--verify-directory cannot be combined with finalization arguments")
